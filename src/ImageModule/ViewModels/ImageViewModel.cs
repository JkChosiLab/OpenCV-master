...
namespace ImageModule.ViewModels
{
    public class ImageViewModel : BindableBase, IDisposable
    {
        ...
        public ImageViewModel(IEventAggregator ea)
        {
            ...
            DeleteImageCommand = new DelegateCommand<CachedImage>(OnDeleteImage);
        }
 

        private void OnDeleteImage(CachedImage img)
        {
            if (img == null)
                return;
        ...
        public DelegateCommand<CachedImage> DeleteImageCommand { get; }
        ...
        private void Compute()
        {
            if (this.SelectedImage == null)
                return;
            // 1) 이전 OperatedImage 정리
            if (OperatedImage != null && !OperatedImage.IsDisposed)
            {
            ...
            {
                using (var copy = this.SelectedImage.GetCopy())
                {
                    if (copy != null)
                        Image = copy.ToBitmapSource();
                    else
                        Image = null;
                }
                return;
            ...
            if (OperatedImage != null && !OperatedImage.Empty())
